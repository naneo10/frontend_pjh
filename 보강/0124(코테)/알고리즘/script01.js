...
function bubbleSort(arr) {
  ...
  for (let i = 0; i < n - 1; i++) { //배열의 n-1번 반복
...
//:찾고자 하는 값이 배열에 존재하면 해당 인덱스를 반환, 존재하지 않으며 -1을 반환

//?문제
//정수 배열 arr와 정수 target이 주어질 때
//, 배열 내에서 target을 찾는 선형 탐색 함수 linearSearch
...
function selectionSort(arr) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    //최대 값과 현재 위치의 요소를 교환
    [arr[i], arr[maxIndex]] = [arr[maxIndex], arr[i]]; //구조분해 할당
  }
  return arr;
}
...
//?주어진 정수 배열 arr를 삽입 정렬 알고리즘을 사용하여 정렬하는 insertionSortBinanrySearch 작성
...
function binarySearch(arr, item, start, end) {
  ...
  if (start >= end) {
    //item이 arr[start]보다 크면 start + 1 그렇지 않으면 그대로의 값을 반환 //*재귀함수
