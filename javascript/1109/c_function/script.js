...
//:코드의 재사용성과 가독성 향상 그리고 디버깅을 용이하게 함 (디버깅=수정하는 작업을 정의)
...
btn.addEventListener('click', function() {
  console.log('Button was cilicked');
});
...
//:함수를 변수에 할당하는 방식
//:함수에 이름을 붙일 수도 있고(기명함수), 붙이지 않을 수도 있다(익명함수)
...
//함수 호출
sum(3, 5); //3, 5는 인수(arguments)

//함수 선언(정의)
function hellow(name) { //name은 매개변수
  console.log(`Hello, ${name} !`);
}

//함수 호출
hellow('SeungAh'); //'SeungAh'는 인수
...
log(); //인수(arguments)를 남기지 X - 출력: underfined
log(1); //인수를 하나 남김 - 출력: 1
log(1, 2); //인수를 두 개 이상 남김 - 출력: 1 (지정된 수의 파라미터 이상의 인수는 읽히지 X)
...
//?1. return 키워드가 없는 함수는 undefined를 반환
...
//?2. 변환 값은 함수의 작업 결과를 함수 외부로 전달하는 방법
...
//반환 값이 있는 함수
function square(x) {
  return x * x; //파라미터(x)의 제곱을 반환
}

console.log(square(5));

//!9. 함수 사용 시 지역변수와 전역변수 사용
