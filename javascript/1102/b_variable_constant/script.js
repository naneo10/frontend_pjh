/* 
  ...
  ! 변수명 명명규칙
    -첫 글자: 영문자, (_)언더스코어, ($)달러만 사용 가능 (이후 숫자 사용가능)
    -띄어쓰기 허용 X
    -기호는 _, $만 사용 가능
    -영어 대소문자 구별
      'now변수'와 'nOw변수'는 다른 변수
    -예약어 사용 X
  ...
  ! 자바스크립트의 변수 선언
    ...
    ?let
      ...
      :재할당 가능
      :호이스팅 'O' (하지만, 선언 전에 접근 불가)

    ?var
      :함수 범위변수(Function Scope)
      :재할당 가능 'O' (선언 부분이 상단으로 끌어올려짐)
*/
...
//! var 키워드의 경우
//변수를 중복해서 선언할 수 있다는 위험성, 변수가 속하는 범위가 애매하는 점에서 코드에 혼란을 줄 수 있기 때문에 사용 'X'