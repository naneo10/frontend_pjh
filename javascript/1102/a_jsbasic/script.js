...
//문장(statement: 진술, 선언)
//:명령, 문장 끝에는 세미클론(;)을 붙이거나 줄바꿈을 넣어서 문장의 종결을 구분
//:하나 이상의 표현식이 모이면 문장
...
//식별자
//:개발자가 자바스크립트의 변수, 함수, 속성 등을 구별하려고 이름붙인 특정 단어를 의미
...
//알파벳(영어)을 사용하는 것이 관례
//'sum = 3 + 5' 와 같이 a, b 처럼 의미없는 단어보다 의미있는 단어 사용을 권장
...
//식별자 뒤에 괄호 X(없는 경우): 변수(단독 사용), 속성(다른 식별자와 사용)
//식별자 뒤에 괄호 O(있는 경우): 함수(단독 사용), 메소드(다른 식별자와 사용)
...
//?4-3.프롬프트 창에서 입출력받기
//:텍스트 필드가 있는 작은 창
//:간단한 메시지 입력가능, 해당 내용을 가져와서 사용 가능
...
//?4-4.웹 브라우저 화면에 출력
//:브라우저 화면에서 결과 값 확인하는 용도의 출력
//document(웹 문서)에서 괄호 안의 내용을 write(표시)하는 명령문
//괄호 안에는 실제 웹 브라우저 화면에 표시할 내용(html 태그 사용 가능), 어떤 결과 값이 저장된 변수 삽입 가능
//document.write();
