...
//:같은 타입이 아닌 여러 값들(여러 개의 변수)을 담을 수 있는 리스트 형태의 객체(같은 타입도 담을 수 있음)
...
//쉼표를 통해 구분되는 각각의 데이터는 요소(argument)
//:요소 - 배열 내부에 들어있는 값
let emptyArray = []; //빈 배열
...
let arrayLenghtTwo = new Array(2); //^요소를 채우는 방법?
...
//^요소추가
let cars = ['bmw', 'audi'];
cars.push('hyundai'); //*배열의 마지막에 삽입
console.log(cars);

//인덱스를 사용하여 배열 뒷부분의 특정 위치에 요소 추가
cars[7] = 'kia'; //*원하는 위치 요소 추가
console.log(cars);
...
let lastCars = cars.pop(); //*마지막 요소 제거
console.log(lastCars); //*'toyota' 마지막 요소 반환
console.log(cars);
...
//splice()
//:배열의 기존 요소를 삭제 또는 교체
//:| 새요소를 추가하여 배열의 내용을 변경

//배열.splice(start, deleteCount, item1, item2 ...)

//start(필수 값): 변경을 시작할 인덱스 번호
...
console.log(cars); //기존 요소 배열 변함 'X'
...
//문자열로 이루어진 데이터를 정제하여
//, 배열의 형태로 나타낼 때 사용
let myData = 'Seoul, Busan, Dadjeon';
