/* 리액트 환경설정
  ...
  ?웹팩(Webpack)
  :모듈(주로 JS 파일) 번들러
  :Bundler(번들러) - 여러 개로 모듈화된 JS 파일을 화나로 합치는 기능의 도구
  
  -로더(Loader)
  :웹팩은 오직 JS와 JSON으로 구성된 팡리만 읽고 변환
  :로더는 다른 유형의 파일(CSS, 이미지 등)을 처리하고 웹팩이 이해할 수 있는 형태로 변환

  ?바벨(Babel)
  :JS 컴파일러, 최신 ES6+ 코드를 이전버전 ES5로 변환하여 브라우저 호환성을 개선

  !CRA(Create React App)
  :리액트 기반 프로젝트를 쉽게 설정할 수 있는 환경 구축도구
  :복잡한 환경설정 없이 리액트 프로젝트를 설치

  ?리엑트를 CRA로 설치하기 전 필수사항
  -Node.js 설치
  -npm 설치
  -코드 IDE 툴 (VSCode)
  ...
  -리엑트의 경우 외부 라이브러리를 많이 사용하기 때문에 용량이 큼
  ...
  :업로드 하고싶지 않은 파일의 확장자 또는 파일명을 작성
