{
  ...
  greet(seungah);
  greet(dokyung);
  // greet(jungook); //Person 타입 속성의 구조와 일치하지 않음 (age 속성 누락)
  ...
  type UserProfile = {
    username: string;
    email: string;
    address?: Address; //Address 타입
  };

  let userA: UserProfile = {
    username: "A",
    email: "adce#naver.com",
    address: {
      street: "123 st",
      city: "Busan",
      zipCode: "12345",
  ...
  type UserRecord = {
    ...
    [key: string]: string;
    // [key: number]: string; 넘버도 지정 가능
  };

  let users: UserRecord = {
    name: "lsa",
    age: "20",
    true: "hello", //*true를 자동으로 string값으로 인지 (숫자도 동일)
    //추가적인 사용자의 속성을 지정 가능
  };
