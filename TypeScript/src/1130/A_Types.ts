{
  ...
  //:typeof 연산자를 사용했을 때 나타나는 변환 값과 동일한 이름을 가짐
  ...
  let list1: string[] = ['1', '2', '3']; //타입을 명시하게 되면 타입에 맞는 배열 요소가 기입되어야 한다
  ...
  //!void 타입
  //:아무런 값이 없다
  //:주로 함수에서 반환 값이 없거나 return이 있더라도 반환하는 값이 없을때 사용되는 타입

  function voidType(parameter: number): void {
    //함수의 타입 정의
    //파라미터와 반환 값 정의가 가능
    //return parameter + parameter;
    console.log('반환 값이 없는 함수');
    return; //undefined
  }
  ...
  //!symbol
  //:전역적으로 고유한 참조 값을 생성하는 데에 사용하는 타입
  //*참조 값 - 주소 값
  //:유일하고 변경 불가능한 값으로 생성
  //:Symbol() 함수로 생성
