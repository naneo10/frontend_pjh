/*
  ...
  ?Windows에서 npm 실행이 되지 않을 경우
    (node.js를 설치 후에도 npm 실행이 안되는 경우)
    ...
    > 고급 > 환경 변수 클릭
      ...
      :사용자 변수에서 Path 설정
        ...
        > 자신의 컴퓨터 이름(ITPS) > AppData >
        Roaming > (1)npm의 경로를 복사

        > Path(변수) 더블클릭 > 환경 변수 편집 창
        > 새로 만들기 > (2)복사한 npm 경로를 삽입
        > 확인 버튼 클릭
  ...
  :프로젝트의 의존성을 관리, 작은 스크립트 실행등의 기능을 제공
  ...
  ?프로젝트 디렉토리 생성
    ...
    1)원하는 위치에서 터미널 오픈
      ...
      *change directory
      ...
      -특정 디렉토리로 이동
        cd directory_경로 ex) cd /c/frontend_0912_pjh, /c/frontend_0912_pjh/TypeScript
      -홈 디렉토리로 이동
        cd | cd ~
      -상위 디렉토리로 이동
        cd ..
  ...
  ?npm 초기화
  :새로운 Node.js 프로젝트를 시작, 기본 값으로 package.json 파일을 생성
    npm init -y
  :package.json
    Node.js 프로젝트의 '메타데이터'를 담고 있는 파일
    *메타데이터: 데이터를 설명하는 데이터
  ...
  devDpendencies: 개발 과정에서만 필요한 패키지들의 목록
