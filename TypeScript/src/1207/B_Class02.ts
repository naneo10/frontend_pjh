{
  ...
  //:반환 값 x, 자신을 new 키워드와 함께 호출하는 대상(클래스)의 새로운 인스턴스를 암시적으로 반환
  ...
  //속성 기본 값
  //:함수의 기본 인자와 유사한 개념으로 클래스 속성에도 기본 값 제공이 가능
  ...
  //:클래스의 멤버변수나 메서드에 접근할 떄 사용

  //?사용 시 주의점
  //:메서드나 생성자 내에서 클래스의 멤버에 접근할 때 'this'를 사용
  //:화살표 함수 내에서 사용
  //:-자신만의 'this'바인딩을 생성하지 x
  //:-해당 함수가 선언도니 범위의 'this'를 사용

  //'this'키워드를 화살표 함수 내에서 사용하는 경우에는 스코프 확인이 필수!

  class Human {
    age: number; //대문자 Number 객체를 지정
    ...
    constructor() {
      ...
      setInterval(() => {
        ...
        //:객체 내에 화살표 함수를 정의하면
        //:해당 함수의 'this'는 외부 스코프(즉, 객체를 둘러싼 스코프)의 'this'를 가리킴
      }, 3000);
    }
  }
  let a = new Human();
