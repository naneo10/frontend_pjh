{
  ...
  //:다른 함수에 인자로 넘겨지는 '함수'
  //?2.사용방법
  //:비동기 작업을 수행하는 함수에 콜백함수를 인자로 전달
  //:비동기 작업이 완료되면, 해당 콜백함수가 호출
  ...
  function fetchData(callback: (data: any) => void) {
    ...
    //fetch() 함수
    //-네트워크 통신을 위해 제공되는 JS 내장함수
    //-주로 웹 API로 부터 데이터를 가져오거나, 데이터를 서버에 전송할 때 사용
    /*
      fetch(url - 요청을 보낼 주소) //*CRUD
        .then((response) => ersponse.json()) //JSON형태로 응답을 처리
      ...
      .then((user) => {
        ...
        fetch(`https://jsonplaceholder.typicode.com/posts?userID=${userId}`)
          ...
          .then((posts) => {
            ...
            //세 번째 요청: 사용자의 엘범 데이터를 가져오기
            fetch(
              `https://jsonplaceholder.typicode.com/albums?userId=${userId}`
            )
              .then((response) => response.json())
              .then((albums) => {
                console.log("사용자의 엘범: ", albums);
                //모든 데이터가 준비되었을 때 콜백함수를 호출
