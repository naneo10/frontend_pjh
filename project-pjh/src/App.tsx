...
//:사용자가 웹앱 내애서 다른 페이지로 이동할 때, 새로운 페이지를 서버로 부터 로드하는 대신 클라이언트 사이드에서 라우팅 처리를 하는 방법

//SPA(싱클 페이지 애플리케이션)구현을 위한 필수적인 도구
...
//?element 속성: 해당 URL과 연결된 컴포넌트 지정
...
function App() {
  return (
    <>
      {/*
        Routes 외에서 작성되는 컴포넌트는 url경로와 상관없이 react 실행 시 렌더링
        :header, nav, footer
      */}
      <nav>
        <ul>
