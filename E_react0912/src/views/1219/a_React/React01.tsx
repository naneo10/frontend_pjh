...
!리액트 동작원리
?CRP 정의
:웹 브라우저가 HTML, CSS, JS 등의리소스를 받아서 픽셀로 변화하여 화면에 렌더링하는 과정

-문서 파싱 및 DOM 생성 (HTML)
:브라우저는 HTML 문서를 받아 파싱하고, 이를 DOM 트리로 변환
:JSX가 Bable을 통해 JS로 변환되어 실행

-CSSOM 생성
:CSS파일과 스타일 태그 CSSOM 트리로 변환

-JS 처리
:컴포넌트 로직과 상태 관리가 이루어짐

-렌더 트리 형성
:DOM과 CSSOM이 결홥되어 렌더 트리가 형성 (화면에 실제 표시될 요소 포함)
