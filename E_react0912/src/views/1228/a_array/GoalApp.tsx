...
//!배열에 항목을 추가
//spread연산자, concat함수를 사용

//!배열에 항목을 제거
//filter 메소드를 사용 (해당 항목과 일치하는 id를 제거)
...
export default function GoalApp() {
  ...
  const handleRemove = (id: number) => {
    ...
    //goal.id가 매개변수로 일치하지 않는 원소만 추출해서 새로운 배열을 만듬
    //:goal.id가 매개변수로 받아오는 id인것을 제거
    setGoals(goals.filter(goal => goal.id !== id));
  }

  const handleToggle = (id: number) => {
    //*true, false 기능이 있음 클릭 시 적용
    ...
    //forEach()
    //:기존 배열(Array)을 변경
    //:단순히 반복문을 대체하기 위한 함수 //*for문 대신 사용

    //:map()
    //:새로운 배열(Array)을 반환
    //:요소 값을 다른 값으로 mapping한(기능을 적용) 새로운 배열을 생성하는 함수
    setGoals(
      goals.map(goal =>
