...
//:usetState에서는 문자열이 아니라 객체 형태의 상태를 관리
...
export default function UseState03() {
  ...
  // const id = input.id;
  // const passoword = input.password;

  //스프레드 연산자
  const obj = { a: 1, b: 2 };
  const newObj = { ...obj, c: 3 }; // { a: 1, b: 2, C: 3};
  const newObj2 = { ...obj, b: 3 }; // { a: 1, b: 3};
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { value, name } = e.target; //*input에서 가져오겠다
    setInputs({
      //스프레드 연산자(...연산자)
      ...
      //   password: ''
      // }
      [name]: value //기존의 input 객체를 복사한 뒤 name 키를 가진 값을 value로 설정 (변경된 필드의 값을 업데이트)
    });
  }
